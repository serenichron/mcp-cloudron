...
describe('cloudron_task_status tool', () => {
  ...
  it('should get task status for successful task with result data', async () => {
    ...
    expect((taskStatus.result as any).backupId).toBe('backup-20241223-140000');
    expect((taskStatus.result as any).size).toBe(1024000000);
  });
  ...
  it('should validate all task states', async () => {
    const states: Array<'pending' | 'running' | 'success' | 'error'> = ['pending', 'running', 'success', 'error'];
