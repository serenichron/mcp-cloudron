...
${errorsText}${warningsText}`,
  ...
  Updated fields: ${updatedFields || 'password'}`,
            ...
            case 'cloudron_list_domains': {
                ...
                const domainList = domains.map(d => `Domain: ${d.domain}
  Zone: ${d.zoneName}
  Provider: ${d.provider}
  TLS: ${d.tlsConfig.provider} (wildcard: ${d.tlsConfig.wildcard})`).join('\n\n');
                return {
                    content: [
                        {
                            type: 'text',
...
Note: Pre-flight validation (F23a + F36) passed. Installation is in progress.`,
            ...
            case 'cloudron_check_updates': {
                ...
                if (!updateInfo.available) {
                    return {
                        content: [{ type: 'text', text: 'Cloudron is up to date. No updates available.' }],
                    };
                }
                return {
                    content: [
...
Use cloudron_apply_update to apply this update. Note: services will restart during update.`,
            ...
            case 'cloudron_apply_update': {
                ...
                if (!updateAvailable.available) {
                    return {
                        content: [{ type: 'text', text: 'No update available. Cloudron is already up to date.' }],
                    };
                }
...
WARNING: Services will restart during the update process. Expect brief downtime.`,
            ...
            case 'cloudron_list_groups': {
                ...
                if (groups.length === 0) {
                    return {
                        content: [{ type: 'text', text: 'No groups found on this Cloudron instance.' }],
                    };
                }
                const groupList = groups
                    ...
                    .join('\n');
                return {
                    content: [{ type: 'text', text: `Groups (${groups.length}):\n\n${groupList}` }],
                };
            }
  ...
  Created: ${group.creationTime}`,
            ...
            case 'cloudron_list_services': {
                ...
                if (services.length === 0) {
                    return {
                        content: [{ type: 'text', text: 'No platform services found.' }],
                    };
                }
                const serviceList = services.map(s => `• ${s.name}: ${s.status}${s.version ? ` (v${s.version})` : ''}${s.memory ? ` — ${Math.round(s.memory / 1024 / 1024)}MB` : ''}`).join('\n');
                return {
                    content: [{ type: 'text', text: `Platform services (${services.length}):\n\n${serviceList}` }],
                };
            }
...
WARNING: This is a DESTRUCTIVE operation. Current app data is being replaced with the backup.`,
            ...
            case 'cloudron_clone_app': {
                const { appId, location, domain, backupId, portBindings } = args;
                const cloneParams = { appId, location };
                if (domain)
                    cloneParams.domain = domain;
                if (backupId)
                    cloneParams.backupId = backupId;
                if (portBindings)
                    cloneParams.portBindings = portBindings;
...
Use cloudron_task_status with taskId '${taskId}' to track update progress.`,
            ...
            case 'cloudron_fetch_package_example': {
                ...
                const sections = [`Package example for: ${appId}\n`];
                if (files.manifest) {
...
- Document changes in CHANGELOG.md`,
                ...
                if (!content) {
                    return {
                        content: [{ type: 'text', text: `Unknown topic: ${topic}. Valid topics: overview, manifest, dockerfile, addons, testing, publishing` }],
                        isError: true,
                    };
                }
                return {
            ...
            case 'cloudron_scaffold_package': {
                const { appName, appType, appId: providedAppId, description: appDescription, version = '1.0.0', httpPort = 8000, addons = ['localstorage'], authMethod = 'none', healthCheckPath = '/', website, memoryLimit = 268435456, } = args;
...
6. Use cloudron_fetch_package_example to see real-world examples`;
            ...
            case 'cloudron_validate_package': {
                const { manifest, dockerfile, startScript } = args;
                const errors = [];
                const warnings = [];
                const suggestions = [];
